...
class _RestaurantsListSectionState extends State<RestaurantsListSection> {
  ...
  void initiateData(BuildContext context) async {
    ...
    final initialData =
        await Provider.of<UserState>(context, listen: false).getInitialData();
    final defaultAddressCoords = await initialData[0];
    final _canShowNotificationToDownloadNewApp = await initialData[1];

    if (_canShowNotificationToDownloadNewApp) showDownloadAppDialog();
  ...
  void showDownloadAppDialog() async {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'A nova versão do app para android está disponível, baixe agora para aproveitar um app mais leve e fluido.',
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(
              'Baixar agora',
              style: AppStyle.pillYellowRegularTextStyle(),
            ),
          ),
        ],
      ),
      ...
      Provider.of<UserState>(context, listen: false)
          .setNewAppNotificationAsSeen();
      if (value != null && value) {
        const url =
            'https://play.google.com/store/apps/details?id=com.tofaminto.client';
